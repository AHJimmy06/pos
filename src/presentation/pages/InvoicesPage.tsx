            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-border/50">
                {invoices.map((invoice) => (
                  <tr key={invoice.id} className="bg-transparent hover:bg-muted/20">
                    <td className="px-6 py-4 font-medium text-muted-foreground">
                      #{invoice.id}
                    </td>
                    <td className="px-6 py-4">{formatDate(invoice.issueDate)}</td>
                    <td className="px-6 py-4 text-right">{formatCurrency(invoice.subtotalSnapshot)}</td>
                    <td className="px-6 py-4 text-right">{formatCurrency(invoice.taxTotalSnapshot)}</td>
                    <td className="px-6 py-4 text-right font-bold">{formatCurrency(invoice.totalSnapshot)}</td>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                {selectedInvoice.transactionId && (
                  <div>
                    ...
                    <p className="font-mono text-xs">{selectedInvoice.transactionId}</p>
                  </div>
                )}
              </div>
                ...
                <div className="rounded-lg border border-border/50 overflow-x-auto">
                  <table className="w-full text-sm min-w-[700px]">
                    <thead className="bg-muted/30">
                      <tr>
                        <th className="px-6 py-3 text-left font-black text-xs whitespace-nowrap">Producto</th>
                        <th className="px-6 py-3 text-right font-black text-xs">Cant.</th>
                        <th className="px-6 py-3 text-right font-black text-xs">Precio Unit.</th>
                        <th className="px-6 py-3 text-right font-black text-xs whitespace-nowrap">
                          IVA {selectedInvoice.details[0]?.taxRate > 0 ? `(${selectedInvoice.details[0].taxRate}%)` : ""}
                        </th>
                        <th className="px-6 py-3 text-right font-black text-xs">Subtotal</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border/50">
                      {selectedInvoice.details?.map((detail, idx) => {
                        const iva = detail.taxRate > 0 ? (detail.subtotal * detail.taxRate) / (100 + detail.taxRate) : 0;
                        return (
                          <tr key={idx}>
                            <td className="px-6 py-4 whitespace-nowrap">{detail.productName}</td>
                            <td className="px-6 py-4 text-right whitespace-nowrap">{detail.quantity}</td>
                            ...
                            <td className="px-6 py-4 text-right whitespace-nowrap text-muted-foreground">
                              {detail.taxRate > 0 ? `${formatCurrency(iva)} (${detail.taxRate}%)` : "-"}
                            </td>
              ...
              <div className="space-y-2 border-t pt-4">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-mono">{formatCurrency(selectedInvoice.subtotalSnapshot)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">IVA</span>
                  <span className="font-mono">{formatCurrency(selectedInvoice.taxTotalSnapshot)}</span>
                </div>
                <div className="flex justify-between text-lg font-black border-t pt-2">
                  <span>Total</span>
                  <span className="font-mono">{formatCurrency(selectedInvoice.totalSnapshot)}</span>
                </div>
              </div>
            </div>
          )}
