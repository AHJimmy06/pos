...
const createFallbackClient = (clientId: number, name: string): DomainClient => {
	const fallback = {
		...
		phone: "",
		address: "",
		email: { value: "N/A", getValue: () => "N/A" },
	};
	return fallback as unknown as DomainClient;
};
					...
					<InvoicePDF
						...
						client={
							...
							createFallbackClient(
								selectedInvoice.clientId || 0,
								getClientName(selectedInvoice.clientId),
							)
						}
						invoiceNumber={
							selectedInvoice.id?.toString().padStart(6, "0") || "N/A"
						}
					/>
				)}
			</div>
						...
						<div className="space-y-4">
							...
							<div className="border border-border rounded-lg overflow-hidden">
								<table className="w-full text-left text-sm">
									<thead className="bg-muted/50 border-b border-border">
										<tr>
											<th className="px-3 py-2 text-xs font-black text-muted-foreground uppercase">
												Producto
											</th>
											<th className="px-2 py-2 text-center text-xs font-black text-muted-foreground uppercase">
												Cant.
											</th>
											<th className="px-3 py-2 text-right text-xs font-black text-muted-foreground uppercase">
												Precio
											</th>
											<th className="px-3 py-2 text-right text-xs font-black text-muted-foreground uppercase">
												Total
									...
									<tbody className="divide-y divide-border/50">
										{selectedInvoice.details?.length ? (
											selectedInvoice.details.map(
												(detail: InvoiceDetail, idx: number) => (
													<tr key={idx}>
							...
							<div className="space-y-1.5 text-sm border-t border-border pt-4">
								<div className="flex justify-between">
									...
									<span className="font-bold">
										{selectedInvoice.subtotal.toString()}
									</span>
								</div>
								<div className="flex justify-between">
									<span className="text-muted-foreground">Impuestos:</span>
									<span className="font-bold text-primary">
										{selectedInvoice.taxTotal.toString()}
									</span>
								</div>
